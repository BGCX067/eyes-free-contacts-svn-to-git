...
package com.google.blindroid.talkingdialer;

import com.google.blindroid.talkingdialer.ShakeDetector.ShakeListener;
import com.google.blindroid.talkingdialer.ContactEntry;
import com.google.blindroid.talkingdialer.FilterableContactsList;
import com.google.blindroid.talkingdialer.ShakeDetector;
import com.google.blindroid.talkingdialer.SlideDial;
import com.google.blindroid.talkingdialer.ContactsView.trackballTimeout;
...
public class ContactsView extends TextView {
  ...
  protected static FilterableContactsList filteredConState;
  ...
  protected static String characterState = "";
  ...
  public ContactsView(Context context) {
    ...
    filteredConState = new FilterableContactsList(contactNames);
    currentString = "";
    currentContact = "";
  ...
  protected void jumpToFirstFilteredResult() {
    ContactEntry entry = filteredConState.next();
 //   if(SlideDial.count !=0 ){entry = filteredConState.previous();}
//    ContactEntry entry = filteredContacts.next();
  ...
  protected void jumpToCurrentFilteredResult() {
	  ContactEntry entry = filteredConState.next();
	  entry = filteredConState.previous();
	    if (entry == null) {
	        parent.tts.speak("No contacts found.", 0, null);
	        if (currentString.length() > 0) {
	          currentString = currentString.substring(0, currentString.length() - 1);
	          characterState = characterState.substring(0, characterState.length() - 1);
	        }
	        return;
	      }
	      managedCursor.moveToPosition(entry.index);
  ...
  protected void speakCurrentContact(boolean interrupt) {
    String name = managedCursor.getString(NAME);
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    ...
    if (input.length() > 0) {
      ...
      characterState = currentString + input;
      filteredConState.filter(characterState);
      jumpToFirstFilteredResult();
    }
    confirmed = false;
    return false;
  ...
  private void confirmEntry() {
    ...
    */
      ...
      characterState = currentString + currentCharacter;
    }
